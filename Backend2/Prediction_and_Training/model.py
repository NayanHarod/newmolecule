...
class MultiGRU(nn.Module):
    ...
    def forward(self, x, h):
        x = self.embedding(x)
        h_out = utils.Variable(torch.zeros(h.size()))
    ...
    def init_h(self, batch_size):
        ...
        return utils.Variable(torch.zeros(3, batch_size, 512))
...
class RNN():
    ...
    def likelihood(self, target):
        ...
        start_token = utils.Variable(torch.zeros(batch_size, 1).long())
        start_token[:] = self.voc.vocab['GO']
        x = torch.cat((start_token, target[:, :-1]), 1)
        h = self.rnn.init_h(batch_size)

        log_probs = utils.Variable(torch.zeros(batch_size))
        entropy = utils.Variable(torch.zeros(batch_size))
    ...
    def sample(self, batch_size, max_length=140):
        """
            ...
            entropy: (batch_size) The entropies for the sequences. Not
                                    currently used.
        """
        start_token = utils.Variable(torch.zeros(batch_size).long())
        start_token[:] = self.voc.vocab['GO']
        h = self.rnn.init_h(batch_size)
        x = start_token
...
def NLLLoss(inputs, targets):
    ...
    loss = utils.Variable(target_expanded) * inputs
